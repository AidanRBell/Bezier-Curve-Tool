...
bool closeEnough(float mouseX, float mouseY, float pointX, float pointY, float maxDist) {
    return maxDist >= sqrt((mouseX - pointX) * (mouseX - pointX) + (mouseY - pointY) * (mouseY - pointY));
...
int main(int argc, char *argv[])
{
    ...
    if (!glfwInit())
    {
        cout << "GLFW didn't initialize" << endl;
        return -1;
    }
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        if (leftClickState == GLFW_PRESS && leftClickPressed) { // left click is being held
            ...
            for(int i = 0; i < points.size(); i++) {
                if (closeEnough(mouseX, mouseY, points[i][0], points[i][1], 9)) {
                    draggingIndex = i;
                    break;
                }
            }
            ...
            if (draggingIndex == -1) {
                for (int j = 0; j < controlPoints.size(); j++) {
                    if (closeEnough(mouseX, mouseY, controlPoints[j][0][0], controlPoints[j][0][1], 9)) {
                        ctrlDraggingIndex = j;
                        ctrlXorY = 0;
                        break;
                    } else if (closeEnough(mouseX, mouseY, controlPoints[j][1][0], controlPoints[j][1][1], 9)) {
                        ctrlDraggingIndex = j;
                        ctrlXorY = 1;
                        break;
                    }
